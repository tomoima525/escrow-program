...
import { EscrowLayout, ESCROW_ACCOUNT_DATA_LAYOUT } from '../layout/escrow';
...
export function createInitEscrowInstruction({
  ...
  const data = new EscrowInitdata(new BN(amount), new BN(fee));
...
export async function createExchangeInstruction({
  ...
  // 4NxQmMd9Z2yCUzrA3JCyrb3vWGnFYLNNX2Qnsq2SbW2e associated account
  const feeTokenAccount = new PublicKey(
    'Ae1c6oswsczoM7YtUdFLUviXMSvKgZTsZ3sJaPTyuVah',
  );
