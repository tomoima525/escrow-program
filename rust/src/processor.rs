...
impl Processor {
  pub fn process(
    ...
    match instruction {
      EscrowInstruction::InitEscrow(args) => {
        ...
        Self::process_init_escrow(program_id, accounts, args.data.amount, args.data.fee)
      }
  ...
  fn process_exchange(program_id: &Pubkey, accounts: &[AccountInfo], amount: u64) -> ProgramResult {
    ...
    msg!(
      "amount {} pda_temp_amount {} fee {}",
      amount,
      pda_temp_token_account_info.amount,
      escrow_info.fee
    );
    if amount != pda_temp_token_account_info.amount - escrow_info.fee {
    ...
    // Perhaps it's better to find an associated account
    let fee_receive_account = next_account_info(account_info_iter)?;
    let fee = escrow_info.fee;

    msg!(
      "Start tranferring fee {} from pda token account {}",
      fee,
      pda_temp_token_account_info.amount
    );
